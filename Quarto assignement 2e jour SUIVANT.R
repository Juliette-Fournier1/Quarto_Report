...
continent_means_water <- map_data_join_2021_with_continent %>%
  filter(!is.na(continent)) %>%
  group_by(continent) %>%
  summarize(mean_water_access = mean(obs_value.x, na.rm = TRUE)) %>%
  filter(continent != "Antarctica")

ggplot(continent_means_water, aes(x = continent, y = mean_water_access)) +
  geom_bar(stat = "identity", fill = "#85C1E9") +
  labs(title = "Mean access to piped drinking water by continent in 2021",
       x = "Continent",
       y = "Proportion of population using piped drinking water sources (%)") +
